...
class EntityExtensionSubscriber implements EventSubscriber
{
     ...
     * @var array<class-string, class-string>
     */
    protected array $allParentClasses = [];
        ...
        foreach ($this->entityExtensionMap as $parentEntity => $extendedEntity) {
            ...
            if (!array_key_exists($parentEntity, $parentClasses)) {
                throw new LogicException(
                    sprintf(
                        'Invalid entity extension mapping. Entity "%s" is not child of "%s"',
                        $extendedEntity,
                        $parentEntity
                    )
                );
            ...
            $this->parentClassesByClass[$extendedEntity] = $parentClasses;
            $this->allParentClasses += $parentClasses;
        }
    ...
    protected function setAssociationMappings(ClassMetadataInfo $classMetadata): void
    {
        ...
        foreach ($this->parentClassesByClass[$currentEntityClass] as $parentClass) {
            if (!$this->isOneOfRegisteredEntities($parentClass)) {
                continue;
            }
    ...
    {
        return array_key_exists($entityClass, $this->parentClassesByClass);
    }
    ...
    {
        return array_key_exists($entityClass, $this->allParentClasses);
    }
    ...
    protected function getOverridingClass(string $className): ?string
    {
        ...
        foreach ($this->parentClassesByClass as $extendedClass => $parentClassesByClass) {
